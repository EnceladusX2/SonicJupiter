...
SUPER, FLY, RESPAWN, HANG, GLIDE, WALLCLIMB, AMYHAMMER, VICTORY}
...
var camLockTime = 0
var camShakeVector = Vector2.ZERO
# var camShakeTime = 0.0
var camShakeDecay = 0.0
...
func _ready():
	...
	camera.drag_left_margin =   camDist.x/viewSize.x/2.1 # dividing this by .21 leads to a more accurate camera trap
	camera.drag_right_margin =  camDist.x/viewSize.x/2.1 # same here
	...
	# once we've got the data needed for the crouch box, we can just delete it
	# maybe in the future we remove it and just use code to find the offset?
...
func _process(delta):
	...
	# Camera shake
	if camShakeVector.x > 0.25 || camShakeVector.y > 0.25:
		camShakeVector.x = snappedf(lerpf(camShakeVector.x, 0, camShakeDecay * (delta/2)), 0.01)
		camShakeVector.y = snappedf(lerpf(camShakeVector.y, 0, camShakeDecay * (delta/2)), 0.01)
		camera.offset = rand_offset()
	else:
		camShakeVector = Vector2.ZERO
	...
	if Global.CDCamera:
		var viewSize = get_viewport_rect().size
		var screenShift = floor(viewSize.x / 5)
		var screenSpeed = floor(screenShift / 32)
		
		if abs(groundSpeed) >= 6*60 or peelOutCharge >= 6*60 or spindashPower > 0:
			if camAdjust.x != screenShift * sign(direction):
				camAdjust.x += screenSpeed * sign(direction)
		else:
			camAdjust.x -= screenSpeed * sign(camAdjust.x)
...
func _physics_process(delta):
	...
	if (camera != null):
		...
		if camLookAmount != 0:
			var camLookScrollSpeed = sign(camLookAmount)*delta*2
			if sign(camLookAmount - camLookScrollSpeed) == sign(camLookAmount):
...
func respawn():
	if partner != null:
		...
		match (partner.character):
			partner.CHARACTERS.TAILS:
			...
			_:
				# Even if the partner isn't Tails, we should banish them to hell temporarily.
				global_position = partner.global_position+Vector2(0, -get_viewport_rect().size.y)
				limitLeft = partner.limitLeft
				limitRight = partner.limitRight
				limitTop = partner.limitTop
...
func lock_camera(time = 1):
	if !Global.CDCamera: # if we want the extended camera from CD, we need to disable camera locking, or else everything gets screwy
...
# This is going to be completely revamped soon
func pose():
	if ground:
		set_state(STATES.VICTORY)

	if partner != null and partner.ground:
		partner.set_state(STATES.VICTORY)

# func shake_screen(strengthVector: Vector2, time: float) -> void:
# 	# The variables provided by the function are just passthroughs
# 	camShakeVector = strengthVector
# 	camShakeTime = time

# 	# If camShakeTime is greater then 0, we should shake our camera until we reach 0
# 	if (camShakeTime > 0):
# 		camera.offset = Vector2(randf_range(-strengthVector.x, strengthVector.x), randf_range(-strengthVector.y, strengthVector.y))
# 		camShakeTime -= 1
# 	else:
# 		# Reset our camera offset to avoid limit clipping
# 		camera.offset = Vector2.ZERO
# 		# Reset our shake vector
# 		camShakeVector = Vector2.ZERO
# 		# Reset our shake timer
# 		camShakeTime = 0.0

## Takes a [Vector2], and moves the camera via the specified pixel amount on each axis.
## **Decay** is the rate which the effect deminishes. High values are faster, and vice versa.
func shake_screen(strengthVector: Vector2, decay: float) -> void:
	# The variables provided by the function are just passthroughs
	camShakeVector = strengthVector
	camShakeDecay = decay

func rand_offset() -> Vector2:
	return Vector2(randf_range(-camShakeVector.x, camShakeVector.x), randf_range(-camShakeVector.y, camShakeVector.y))