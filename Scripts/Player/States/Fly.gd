...
func _physics_process(delta):
	...
	if parent.movement.y >= -1*60 and flightTime > 0 and !parent.roof and parent.position.y >= parent.limitTop+16:
		if parent.inputs[parent.INPUTS.ACTION] > 0 and (!actionPressed or parent.get_y_input() < 0) and (carryBox.get_player_contacting_count() == 0 or !parent.water):
			flyGrav = -0.125
	...
	# recurl into a ball if down and action 2 are pressed
	if (parent.is_down_held() and parent.inputs[parent.INPUTS.ACTION2] > 0):
