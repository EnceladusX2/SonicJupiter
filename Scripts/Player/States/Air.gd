...
func _process(_delta):
	if parent.playerControl != 0 or (parent.inputs[parent.INPUTS.YINPUT] < 0 and parent.character == parent.CHARACTERS.TAILS):
		...
		# Shield actions
			...
			# Super actions
			if parent.isSuper and (parent.character == parent.CHARACTERS.AMY):
...
func _physics_process(delta):
	...
	if (isJump):
		...
		# Drop dash (for amy)
		# Sonic has had his Dropdash removed. I want to keep Amy's hammer swing though
		if parent.character == parent.CHARACTERS.AMY:
	...
	if (parent.ground):
		...
		if !bounce():
			...
			# Drop dash release (for amy)
			if dropTimer >= 1 and (parent.character == parent.character == parent.CHARACTERS.AMY):
