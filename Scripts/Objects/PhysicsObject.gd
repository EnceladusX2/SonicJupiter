...
func _physics_process(delta):
	...
	while (!moveRemaining.is_equal_approx(Vector2.ZERO) or checkOverride) and !translate:
		...
		if backSensor.is_colliding():
			#Calculate the move distance vector, then move
			var rayHitVec = (backSensor.get_collision_point()-backSensor.global_position)
			var normalHitVec = -Vector2.LEFT.rotated(snap_angle(rayHitVec.normalized().angle()))
			position += (rayHitVec-(normalHitVec*(pushRadius)))
