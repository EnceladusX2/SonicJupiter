...
@onready var timeMinutes = $Counters/Text/TimeMinutes
@onready var timeSeconds = $Counters/Text/TimeSeconds
@onready var timeMilliseconds = $Counters/Text/TimeMilliseconds
...
func _process(delta):
	...
	timeMinutes.text = "%1d" % floor(timeClamp/60)
	timeSeconds.text = str(fmod(floor(timeClamp),60)).pad_zeros(2)
	timeMilliseconds.text = str(fmod(floor(timeClamp*100),100)).pad_zeros(2)
