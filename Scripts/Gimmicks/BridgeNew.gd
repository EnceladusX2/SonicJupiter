...
func _physics_process(delta):
	if !Engine.is_editor_hint():
		...
		for i in player:
			if i.movement.y >= 0 and hasCollided:
				playerTouch = true
		...
		# This is basically a catch to double check the player has been removed from the array
		for i in playerLog:
			if player.has(i) and hasCollided and playerTouch:
				hasCollided = false
				player.erase(i)
				playerLog.erase(i)
...
func handle_collision(body):
	if !player.has(body) and player.size() < 1 and !hasCollided:
		hasCollided = true
		player.append(body)
		playerLog.append(body)
	elif player.has(body) and !body.objectCheck.is_colliding():
		#try to erase the player body here, this tends to fail
		hasCollided = false
		player.erase(body)
